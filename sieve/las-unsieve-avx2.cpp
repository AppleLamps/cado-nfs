...
static const __m256i even_masks[2] = {
...
sieve_info_test_lognorm_avx2_mask(__m256i * S0, const __m256i pattern0,
                                  const __m256i * S1, const __m256i pattern1)
{
    __m256i const a = *S0;
    __m256i const r = *S1;
    ...
    *S0 = _mm256_or_si256(a, _mm256_xor_si256(m1, ff));
    return (unsigned int) _mm256_movemask_epi8(m1);
}

static inline unsigned int
sieve_info_test_lognorm_avx2_mask_oneside(__m256i * S0, const __m256i pattern0)
{
    __m256i const a = *S0;
        ...
        std::vector<uint32_t> &survivors)
{
    ...
    {
        const unsigned int mask = sieve_info_test_lognorm_avx2_mask(
                    (__m256i *) (SS[0] + x_start), patterns[0],
                    (__m256i *) (SS[1] + x_start), patterns[1]);
        ...
        std::vector<uint32_t> &survivors)
{
    ...
    for (int x_start = 0; x_start < (i1 - i0); x_start += x_step)
    {
        const unsigned int mask = sieve_info_test_lognorm_avx2_mask_oneside(
                    (__m256i *) (SS + x_start), pattern);
